...
public class DynamicDataSourceAspect {
    ...
    public void changeDataSource(JoinPoint point, DataSourceAnnotation dataSourceAnnotation) throws Throwable {
        ...
        if (!DynamicDataSourceContextHolder.containsDataSource(dsId)) {
            System.err.println("数据源[{}]不存在，使用默认数据源 > {}"+dataSourceAnnotation.value()+point.getSignature());
        } else {
            System.out.println("Use DataSource : {} > {}"+dataSourceAnnotation.value()+point.getSignature());
            //找到的话，那么设置到动态数据源上下文中。
            DynamicDataSourceContextHolder.setDataSource(dataSourceAnnotation.value());
        }
    }
